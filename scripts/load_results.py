...
def load_results():
    ...
    plot_again = True
    exps = [hallway_file, habitat_file, vlm_file, hallway_early_stopping_1, hallway_early_stopping_2, hallway_early_stopping_3]
    labels = ["hallway", "habitat", "vlm", "early stopping 1", "early stopping 2", "early stopping 3"]
    for exp, label in zip(exps, labels):
        ...
        if plot_again:
            ...
            img_coverage = plot_prediction_set_size_versus_success(
                ...
                no_help_agg_task_success_rate, calibration_temps=None,
            )
            img_coverage.save(new_filepath)

        target_val = 0.8 if not label == "vlm" else 0.8
...
def get_intermediates_lin_interp(agg_task_success_rate, agg_help_rate, agg_task_success_rate_stage, agg_help_rate_stage,
                                 target_val=0.85, interp_on=False):
...
def get_closest_index_success(success_rates, target=0.85):
    ...
    over = success_rates[zero_crossings]

    i_under = zero_crossings + 1
    i_over = zero_crossings
