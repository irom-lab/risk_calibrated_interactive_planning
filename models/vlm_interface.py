...
from models.bimanual_planner import powerset
...
PROMPT_VLM_ONLY = ("Us: You are a robot helping a human sort some items. Please select the bin that you would place the"
                    "object in. A) White bin B) Black Bin C) Blue Bin D) Other bin (not shown). Just respond with a "
                   "single token: A B C D. You: ")

PROMPT_BIMANUAL = ("Us: You are a robot helping a human sort some items. The human has given the following "
                   "instruction: ")

MCQA_BIMANUAL = ("Please select the bin that you would place the"
                    "object in. A) Left Black Bin B) Left Blue Bin C) Center White Bin D) Left Blue Bin E) Left Black Bin."
                   "Just respond with a single token: A B C D E. You: ")
 
PROMPT_LLM = ("Here is a description of three bins and an object we want to sort.")

OPTIONS = ["Bin 1", "Bin 2", "Bin 3", "Unsure"]
LETTER_CHOICES = [" A", " B", " C", " D"]
LETTER_CHOICES_SINGLE = ["A", "B", "C", "D"]
LETTER_CHOICES_PAREN = ["A)", "B)", "C)", "D)"]
...
BM_LETTER_CHOICES_SINGLE = list(string.ascii_uppercase)
BM_LETTER_CHOICES = [f" {c}" for c in BM_LETTER_CHOICES_SINGLE]
BM_LETTER_CHOICES_PAREN = [f"{c})" for c in BM_LETTER_CHOICES_SINGLE]



PROMPT_DEBUG = "This is a set of four distinct images. What's in each image? List all objects you see."
def make_bimanual_choices():

    bin_power_set = powerset([1, 2, 3, 4, 5])
    bins = ["Left Black", "Left Blue", "Center White", "Right Blue", "Right Black"]

    letter = 'A'
    letters = [letter]
    mcqa = "Which option seems best: "
    for bin_ind in bin_power_set[1:27]:
        if len(bin_ind) == 1:
            mcqa += f"{letter}) {bins[bin_ind[0]-1]} bin, "
            letter = next_alpha(letter)
            letters.append(letter)
        else:
            bins_indexed = [bins[i-1] for i in bin_ind]
            bins_str = ", ".join(bins_indexed)
            mcqa += f"{letter}) Any of the {bins_str} bins, "
            letter = next_alpha(letter)
            letters.append(letter)
    letters_str = " ".join(letters)
    mcqa += f"? Enter your answer as a single token: {letters_str}"
    return mcqa



def get_mcqa(include_none=True, shuffle_options=False, single_letter=True, python_dict_probs=False, explain=False):
...
def generate_prediction_openai_vision_only(prompt,
                               image_files,
                               indices,
                               max_tokens=1000,
                               temperature_vlm=0,
    ...
    if full_message is None or scores is None:
        ...
        # print(scores)
        save_plan(image_files, full_message, scores, temperature_vlm)
    get_human_location = True
    if get_human_location:
        loc_prompt = "Us: What side is the human hand? Enter left, right, or center. You: "
        loc_message, _ = vlm_or_llm(loc_prompt, image_files, max_tokens=10,
                                          temperature=temperature_vlm, seed=seed, is_vlm=True,
                                          use_bimanual=True)
        if 'right' in loc_message.lower():
            full_message = 'right'
        elif 'left' in loc_message.lower():
            full_message = 'left'
        else:
            full_message = 'center'
...
def vlm_or_llm(prompt,
    ...
    for _ in range(max_attempts):
        try:
            ...
            parsed_response = parse_response(response, logprobs_avail=True, use_bimanual=use_bimanual)
            break
        except Exception as e:
            print(e)
...
def parse_response(response, logprobs_avail=False, python_dict_probs=False, use_bimanual=False):
    ...
    if not use_bimanual:
        ...
        letter_choices_single = LETTER_CHOICES_SINGLE
        letter_choices_paren = LETTER_CHOICES_PAREN
    else:
        letter_choices = BM_LETTER_CHOICES
        letter_choices_single = BM_LETTER_CHOICES_SINGLE
        letter_choices_paren = BM_LETTER_CHOICES_PAREN
    if logprobs_avail:
        logprobs_tensor = -10*torch.ones(len(letter_choices))
        logprobs_choices = []
        for i in range(len(letter_choices)):
            logprobs_choices.append([])
        ...
        for t, l in token_prob_list:
            if t in letter_choices:
                ...
                logprobs_choices[i].append(l)
            if t in letter_choices_single:
                i = letter_choices_single.index(t)
                logprobs_choices[i].append(l)
            if t in letter_choices_paren:
                i = letter_choices_paren.index(t)
                logprobs_choices[i].append(l)
        for i in range(len(logprobs_choices)):
            if len(logprobs_choices[i]) > 0:
                logprobs_tensor[i] = max(logprobs_choices[i])
...
def get_action_distribution_from_image(args, image_path, object_ind, temperature_llm, use_llava=False, llava_args=None,
                                       debug=False, use_bimanual=False, instruction=None):
    ...
    if not use_bimanual:
        prompt = PROMPT_VLM_ONLY
    else:
        prompt = PROMPT_BIMANUAL + instruction + ". " + make_bimanual_choices()
    ...
    if use_llava:
        ...
        pred, score, text = generate_prediction_openai_vision_only(prompt,
                                                 image_path,
                                                 indices,
